 ...
 * to extract a valid { score, feedback } pair.
 *
 * Levels:
...
export function parseAndValidateResponse(data: unknown): {
  ...
  if (typeof data === "object" && data !== null) {
    ...
    if (
      ...
      return { score: Math.round(score), feedback: obj.feedback };
    }
  ...
  // Level 4 – extract the first number that looks like a score (0-100)
  const scoreMatch = data.match(/(\d+(?:\.\d+)?)/);
  if (scoreMatch) {
    const score = Number(scoreMatch[1]);
    if (Number.isFinite(score) && score >= 0 && score <= 100) {
      return { score: Math.round(score), feedback: data };
    }
  }
